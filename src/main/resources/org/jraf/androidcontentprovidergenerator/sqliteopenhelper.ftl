...
public class ${config.sqliteOpenHelperClassName} extends SQLiteOpenHelper {
    ...
    public static final String SQL_CREATE_TABLE_${entity.nameUpperCase} = "CREATE TABLE IF NOT EXISTS "
            ...
            <#if config.enableForeignKeys >
                <#list entity.fields as field>
                    <#if field.foreignKey??>
            + ", CONSTRAINT fk_${field.nameLowerCase} FOREIGN KEY (" + ${entity.nameCamelCase}Columns.${field.nameUpperCase} + ") REFERENCES ${field.foreignKey.entity.nameLowerCase} (" + ${field.foreignKey.entity.nameCamelCase}Columns._ID + ") ON DELETE ${field.foreignKey.onDeleteAction.toSql()}"
                    </#if>
                </#list>
            </#if>
